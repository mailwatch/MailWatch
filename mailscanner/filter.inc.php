...
class Filter
{
    ...
    public function __construct()
    {
        ...
        $this->columns = [
            ...
            'spamallowlisted' => __('spamwhitelisted09'),
            'spamblocklisted' => __('spamblacklisted09'),
            'sascore' => __('sascore09'),
            'spamreport' => __('spamreport09'),
            'ismcp' => __('ismcp09'),
            'ishighmcp' => __('ishighmcp09'),
            'issamcp' => __('issamcp09'),
            'mcpallowlisted' => __('mcpwhitelisted09'),
            'mcpblocklisted' => __('mcpblacklisted09'),
...
" . $this->CreateSQL();
    ...
    public function CreateMtalogSQL()
    {
        ...
        foreach ($this->item as $val) {
            if ('date' === $val[0]) {
                $sql .= self::mtalogDateCondition($val[1], $val[2]);
            }
        }
    ...
    private static function mtalogDateCondition($operator, $day)
    {
        ...
        switch ($operator) {
            ...
            case '<>':
                return "AND\n (timestamp < '$from' OR timestamp >= '$to')\n";
            case '<':
                return "AND\n timestamp < '$from'\n";
            case '<=':
                return "AND\n timestamp < '$to'\n";
