...
class PEAR_PackageFile_v2_rw extends PEAR_PackageFile_v2
{
    ...
    http://pear.php.net/dtd/package-2.0.xsd',
        ...
        if (!isset($this->_packageInfo['name'])) {
            return $this->_packageInfo = array_merge(['name' => $package],
                $this->_packageInfo);
        }
    ...
    function setPhpDep($min, $max = false, $exclude = false)
    {
        ...
        if (isset($this->_packageInfo['dependencies']['required']['php'])) {
            $this->_stack->push(__FUNCTION__, 'warning', ['dep' =>
    ...
    function setPackageType($type)
    {
        ...
        if (!isset($this->_packageInfo[$type])) {
            ...
            $this->_packageInfo = $this->_insertBefore($this->_packageInfo, ['changelog'],
                [], $type);
        }
    ...
    function addInstallAs($path, $as)
    {
        ...
        $this->_isValid = 0;
        $r = $this->_mergeTag($r, ['attribs' => ['name' => $path, 'as' => $as]],
            [
                'filelist' => [],
                'install' => ['ignore']
            ]);
    ...
    function addIgnore($path)
    {
        ...
        $this->_isValid = 0;
        $r = $this->_mergeTag($r, ['attribs' => ['name' => $path]],
            [
                'filelist' => [],
                'ignore' => []
            ]);
