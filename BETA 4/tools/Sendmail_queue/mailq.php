...
if(flock($fl, LOCK_EX + LOCK_NB)) {
 ...
 foreach($queue as $table_name=>$queuedir) {
  ...
  if($dh=@opendir($queuedir)) {
   while(false !== ($file = readdir($dh))) {
        ...
        if(($line = @fgets($fh,1024))) {
         switch(true) {
          ...
          case preg_match('/^(\d{10,}) \d+$/',$line,$match):
           ...
           $output[$msgid]['cdate'] = $ctime['year']."-".str_pad($ctime['mon'],2,"0",STR_PAD_LEFT)."-".str_pad($ctime['mday'],2,"0",STR_PAD_LEFT);
           $output[$msgid]['ctime'] = str_pad($ctime['hours'],2,"0",STR_PAD_LEFT).":".str_pad($ctime['minutes'],2,"0",STR_PAD_LEFT).":".str_pad($ctime['seconds'],2,"0",STR_PAD_LEFT);
           break;
       ...
       if(($fh = @fopen($MsgDir.$msgid, "r"))) {
        ...
        while(!@feof($fh)) {
         if(($line = @fgets($fh,1024))) {
		  ...
		  $output[$msgid]['message'] = preg_replace("/<br \/>/","<BR>",$output[$msgid]['message']);
         }
        }
        fclose($fh);
    ...
    if(preg_match("/^qf/",$file)) {
     ...
     if(($fh = @fopen($queuedir.$file, "r"))) {
      // Work out the total size (df+qf) of the mail
      $output[$msgid]['size'] = (@filesize($queuedir."df".$msgid) + @filesize($queuedir."qf".$msgid));
      while(!@feof($fh)) {
       if(($line = @fgets($fh,1024))) {
        switch(true) {
  ...
  if(($output)) foreach($output as $msgid=>$msginfo) {
   // Insert each record
